...
func Start(logger *slog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			_ = c.Error(common_errors.NewInternalServerError(errors.New("error starting LSP server")))
			return
		}
...
func Stop(logger *slog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			_ = c.Error(common_errors.NewInternalServerError(errors.New("error stopping LSP server")))
			return
		}
...
func DocumentSymbols(logger *slog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if languageId == "" {
			_ = c.Error(common_errors.NewBadRequestError(errors.New("languageId is required")))
			return
		}

		pathToProject := c.Query("pathToProject")
		if pathToProject == "" {
			_ = c.Error(common_errors.NewBadRequestError(errors.New("pathToProject is required")))
			return
		}

		uri := c.Query("uri")
		if uri == "" {
			_ = c.Error(common_errors.NewBadRequestError(errors.New("uri is required")))
			return
		}
...
func WorkspaceSymbols(logger *slog.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		query := c.Query("query")
		if query == "" {
			_ = c.Error(common_errors.NewBadRequestError(errors.New("query is required")))
			return
		}
