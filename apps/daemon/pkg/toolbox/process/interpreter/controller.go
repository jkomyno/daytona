...
func (c *Controller) CreateContext(ctx *gin.Context) {
	...
	if err != nil {
		_ = ctx.Error(common_errors.NewInvalidBodyRequestError(fmt.Errorf("invalid request payload: %w", err)))
		return
	}
	...
	if language != LanguagePython {
		_ = ctx.Error(common_errors.NewCustomError(http.StatusBadRequest, fmt.Sprintf("unsupported language: %s (only '%s' is supported currently)", language, LanguagePython), string(common.CodeProcessInvalidCommand)))
		return
	}
...
func (c *Controller) DeleteContext(ctx *gin.Context) {
	id := ctx.Param("id")
	if id == "" {
		_ = ctx.Error(common_errors.NewBadRequestError(errors.New("context ID is required")))
		return
	}

	if id == "default" {
		_ = ctx.Error(common_errors.NewBadRequestError(errors.New("cannot delete default context")))
		return
	}
	...
	if err != nil {
		if common_errors.IsNotFoundError(err) {
			_ = ctx.Error(common_errors.NewCustomError(http.StatusNotFound, err.Error(), string(common.CodeProcessNotFound)))
			return
		}
		_ = ctx.Error(common_errors.NewInternalServerError(err))
		return
