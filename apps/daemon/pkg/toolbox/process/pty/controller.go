...
func (p *PTYController) CreatePTYSession(c *gin.Context) {
	...
	// Validate session ID
	if req.ID == "" {
		_ = c.Error(common_errors.NewBadRequestError(errors.New("session ID is required")))
		return
	}

	// Check if session with this ID already exists
	if _, exists := ptyManager.Get(req.ID); exists {
		_ = c.Error(common_errors.NewConflictError(fmt.Errorf("PTY session with ID '%s' already exists", req.ID)))
		return
	}
	...
	if *req.Cols > 1000 {
		_ = c.Error(common_errors.NewCustomError(http.StatusBadRequest, "invalid value for cols - must be less than 1000", string(common.CodeProcessInvalidCommand)))
		return
	}
	if *req.Rows > 1000 {
		_ = c.Error(common_errors.NewCustomError(http.StatusBadRequest, "invalid value for rows - must be less than 1000", string(common.CodeProcessInvalidCommand)))
		return
	}
	...
	if !req.LazyStart {
		if err := session.start(); err != nil {
			...
			_ = c.Error(common_errors.NewInternalServerError(errors.New("failed to start PTY session")))
			return
		}
	}
...
func (p *PTYController) GetPTYSession(c *gin.Context) {
	...
	_ = c.Error(common_errors.NewCustomError(http.StatusNotFound, "PTY session not found", string(common.CodeProcessNotFound)))
}
...
func (p *PTYController) ResizePTYSession(c *gin.Context) {
	...
	if req.Cols > 1000 {
		_ = c.Error(common_errors.NewCustomError(http.StatusBadRequest, "cols must be less than 1000", string(common.CodeProcessInvalidCommand)))
		return
	}
	if req.Rows > 1000 {
		_ = c.Error(common_errors.NewCustomError(http.StatusBadRequest, "rows must be less than 1000", string(common.CodeProcessInvalidCommand)))
		return
	}

	session, err := ptyManager.VerifyPTYSessionForResize(id)
	if err != nil {
		_ = c.Error(common_errors.NewCustomError(http.StatusGone, err.Error(), string(common.CodeSessionEnded)))
		return
	}

	if err := session.resize(req.Cols, req.Rows); err != nil {
		_ = c.Error(common_errors.NewInternalServerError(err))
		return
	}
