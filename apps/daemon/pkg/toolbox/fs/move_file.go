...
func MoveFile(c *gin.Context) {
	...
	if sourcePath == "" || destPath == "" {
		_ = c.Error(common_errors.NewBadRequestError(errors.New("source and destination paths are required")))
		return
	}
	...
	if err != nil {
		_ = c.Error(common_errors.NewCustomError(http.StatusBadRequest, "invalid source path", string(common.CodeInvalidFilePath)))
		return
	}

	absDestPath, err := filepath.Abs(destPath)
	if err != nil {
		_ = c.Error(common_errors.NewCustomError(http.StatusBadRequest, "invalid destination path", string(common.CodeInvalidFilePath)))
		return
	}
	...
	if _, err := os.Stat(absDestPath); err == nil {
		_ = c.Error(common_errors.NewConflictError(errors.New("destination already exists")))
		return
	}
	...
	if err != nil {
		...
		if err := copyFile(absSourcePath, absDestPath, sourceInfo); err != nil {
			_ = c.Error(common_errors.NewBadRequestError(fmt.Errorf("failed to move file: %s", err.Error())))
			return
		}
