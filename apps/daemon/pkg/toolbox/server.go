...
func (s *server) Start() error {
	...
	errMiddleware := common_errors.NewErrorMiddleware("DAYTONA_DAEMON", func(ctx *gin.Context, err error) common_errors.ErrorResponse {
		// Status-only classification (no code) for unknown errors. Handlers that
		// need a structured code wrap their errors in *CustomError themselves
		// (see e.g. pkg/toolbox/git/errors.go), bypassing this default branch.
