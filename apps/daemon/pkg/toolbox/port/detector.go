...
func (d *portsDetector) IsPortInUse(c *gin.Context) {
	...
	if err != nil {
		_ = c.Error(common_errors.NewBadRequestError(errors.New("invalid port: must be a number between 3000 and 9999")))
		return
	}

	if port < 3000 || port > 9999 {
		_ = c.Error(common_errors.NewBadRequestError(errors.New("port out of range: must be between 3000 and 9999")))
		return
	}
