...
const sandboxViewSearchParams = {
  ...
  sandboxClass: parseAsArrayOf(parseAsString).withDefault([]),
  labels: labelsParser,
  minCpu: parseAsFloat,
  maxCpu: parseAsFloat,
...
const Sandboxes: React.FC = () => {
  ...
  const filters = useMemo<SandboxFilters>(() => {
    ...
    const sandboxClasses = getValidatedSandboxClasses(viewParams.sandboxClass)

    if (search) nextFilters.name = search
    if (states.length > 0) nextFilters.states = states
    if (sandboxClasses.length > 0) nextFilters.sandboxClass = sandboxClasses
  ...
  const handleFiltersChange = useCallback(
    (newFilters: SandboxFilters) => {
      ...
      setViewParams({
        ...
        sandboxClass: newFilters.sandboxClass?.length ? newFilters.sandboxClass : null,
