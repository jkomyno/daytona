...
func (r SandboxAPIListSandboxesRequest) SandboxClass(sandboxClass []SandboxClass) SandboxAPIListSandboxesRequest {
	r.sandboxClass = &sandboxClass
	return r
}
...
func (a *SandboxAPIService) ListSandboxesExecute(r SandboxAPIListSandboxesRequest) (*ListSandboxesResponse, *http.Response, error) {
	...
	if r.sandboxClass != nil {
		t := *r.sandboxClass
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "sandboxClass", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "sandboxClass", t, "form", "multi")
		}
	}
	if r.minCpu != nil {
